...
1. Is the extremum `flow` rate affected by both the `year` or `season` of the record, and the special conditions `sym`?
...
- Now we can generate a plot we 

``` {r}
sym.labs <- c('B: Ice condtions','E: Estimated','NA: No additional info')
names(sym.labs) <- c("B", "E", "NA")
flow_sample_season  %>% 
  filter(extreme_type == "minimum")  %>% 
  ggplot(aes(flow, year, colour = season)) +
  geom_point(size = 1.5, alpha = 0.8) +
  scale_x_log10(labels = scales::label_number(suffix = "m^3/s")) +
  labs(x = "Flow Rate", y = "Year", colour = "Season") +
  facet_wrap(~ sym, ncol = 2, labeller = labeller(sym = sym.labs)) +
  theme_bw() 
```
