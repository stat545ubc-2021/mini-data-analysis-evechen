...
2. Is there a trend (increases or decreases) of the difference between `maximum` and `minimum` flow rate over time (in `years`)?
3. If there is a trend, is it related to the `day` or `month` when the flow rate is recorded? - Could it because they have recorded some maximum flow rate on days when it was not really maximum?
